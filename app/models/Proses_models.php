...
public function edit_user($data)
{
    ...
    $nis = htmlspecialchars($data['nis'],  ENT_QUOTES);
    $kelas = htmlspecialchars($data['kelas'],  ENT_QUOTES);
    $jurusan = htmlspecialchars($data['jurusan'],  ENT_QUOTES);
    $username = htmlspecialchars($data['username'],  ENT_QUOTES);
    $password = htmlspecialchars($data['password'],  ENT_QUOTES);
    $password_baru = htmlspecialchars($data['password_baru'],  ENT_QUOTES);
    $con_password = htmlspecialchars($data['con_password'],  ENT_QUOTES);
    $id = htmlspecialchars($data['id'],  ENT_QUOTES);

      if (empty($password_baru) && empty ($con_password)) {
        try{
           $query = "UPDATE auth SET nama = :nama, nis = :nis, kelas = :kelas, username = :username ,id_jurusan = :id_jurusan WHERE id_auth = :id_auth";
          $this->db->query($query);
          $this->db->bind('nama', $nama);
          $this->db->bind('nis', $nis);
          $this->db->bind('kelas', $kelas);
          $this->db->bind('id_jurusan', $jurusan);
          $this->db->bind('username', $username);
        ...
        $PWL = $this->model('Get_models')->ambilDataBy('id_auth',$id,'auth');
        $password_lama = $PWL['password'];
        if (password_verify($password, $password_lama)) {
          if ($password_baru === $con_password) {
              $password_baruV2 = password_hash($con_password, PASSWORD_DEFAULT);
              try{
                $query = "UPDATE auth SET nama = :nama, nis = :nis, kelas = :kelas, username = :username, password = :password ,id_jurusan = :id_jurusan WHERE id_auth = :id_auth";
                $this->db->query($query);
                $this->db->bind('nama', $nama);
                $this->db->bind('nis', $nis);
                $this->db->bind('kelas', $kelas);
              ...
              Flasher::setFlash('Password baru harus sama ',' ','error');
             header('Location: '. BASEURL . '/user/edit_user/'.$id);
             exit();
           }
          }else{
